    ...
    const results = await Promise.allSettled(
      selectedRows.map(async (row) => {
        const attributeValues: Record<number, string> = {
          [selectedAttribute.uuid]: value,
        };
        return updateParticipant(eventUuid, row.original.uuid, {
            ...
            >
              ...
              <SelectContent>
                {editableAttributes.map((attr) => (
                  <SelectItem key={attr.uuid} value={attr.uuid}>
                    {attr.name}
                  </SelectItem>
                ))}
              </SelectContent>
          ...
          {selectedAttribute != null && (
            <div className="flex flex-col gap-2">
              ...
              <AttributeValueInput
                ...
                value={value}
                onChange={setValue}
                idPrefix={`bulk-${selectedAttribute.uuid}`}
              />
            </div>
          )}
        </div>
