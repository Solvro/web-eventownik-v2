...
import { AttributeStatistics } from "@/app/dashboard/events/[id]/statistics/attribute-statistics";
import { AttributeStatisticsPicker } from "@/app/dashboard/events/[id]/statistics/attribute-statistics-picker";
...
export default function StatisticsPage() {
  ...
  const { data: attributes } = useQuery(eventAttributesQueryOptions(id));

  const [selectedAttributeId, setSelectedAttributeId] = useState<string>();

  // Derive the effective selection so we don't need an effect to seed a default.
  const selectedId = selectedAttributeId ?? attributes?.[0]?.id.toString();
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <Activity mode={statistic === "attributes" ? "visible" : "hidden"}>
        <AttributeStatistics eventId={id} selectedId={selectedId} />
      </Activity>
